...
function SessionStartBlock({ event }: { event: SessionStartEvent }) {
  return (
    <div className="py-3 border-b border-[#1f1f1f] mb-1">
      <div className="text-[11px] text-[#3f3f46] font-mono mb-1.5">◆ session init</div>
      <div className="flex flex-wrap items-center gap-x-2.5 gap-y-0.5 text-[12px] text-[#71717a]">
...
function FramingBlock({ event }: { event: FacilitatorFramingEvent }) {
  return (
    <div className="py-4 border-b border-[#1f1f1f] mb-1">
      <div className="text-[11px] text-[#3f3f46] font-mono mb-2.5">⬡ framing</div>
      <p className="text-[13px] text-[#d4d4d8] leading-relaxed mb-3">{event.definition}</p>
      {event.questions.length > 0 && (
        <div>
          <div className="text-[10px] uppercase tracking-widest text-[#52525b] mb-1.5">
            Key questions
          </div>
          <ul className="space-y-1">
            {event.questions.map((q, i) => (
              <li key={i} className="flex gap-2 text-[12px] text-[#a1a1aa]">
                <span className="text-[#3f3f46] shrink-0 mt-px">·</span>
                <span>{q}</span>
              </li>
            ))}
          </ul>
      ...
      <div className="mt-3 text-[10px] text-[#3f3f46] font-mono">
...
function RoundStartBlock({ event }: { event: RoundStartEvent }) {
  return (
    <div className="flex items-center gap-3 py-4">
      <span className="text-[11px] text-[#3f3f46] font-mono whitespace-nowrap">
        ▶ round {event.round}
      </span>
...
function ThinkingBlock({ event }: { event: AgentThinkingEvent }) {
  return (
    <div className="py-2 pl-3 border-l border-[#27272a] mb-0.5">
      <span className="text-[12px] text-[#52525b]">{event.agent}</span>
      <span className="text-[12px] text-[#3f3f46] mx-1.5">—</span>
      <span className="text-[12px] text-[#3f3f46]">thinking</span>
      <span className="dot-blink text-[12px] text-[#3f3f46]">.</span>
      <span className="dot-blink text-[12px] text-[#3f3f46]">.</span>
      <span className="dot-blink text-[12px] text-[#3f3f46]">.</span>
    </div>
  )
  ...
  return (
    <div className="py-4 pl-3 border-l-2 border-[#27272a] mb-2">
      <div className="flex items-baseline gap-2 mb-2.5">
        ...
        <span className="text-[12px] text-[#71717a]">{event.role}</span>
...
function ExtractionBlock({ event }: { event: RoundExtractionEvent }) {
  return (
    <div className="py-3 ml-3 mb-2">
      <div className="text-[10px] text-[#3f3f46] font-mono mb-1.5">
...
function SessionEndBlock({ event }: { event: SessionEndEvent }) {
  return (
    <div className="py-3 mt-2 text-[11px] text-[#3f3f46] font-mono">
